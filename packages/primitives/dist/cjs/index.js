...
function createDisclosure(props = {}) {
  const [isOpen, setIsOpen] = createControllableBooleanSignal({
    value: () => utils.access(props.isOpen),
    defaultValue: () => !!utils.access(props.defaultIsOpen),
...
function createPreventScroll(options) {
  solidJs.createEffect(solidJs.on(() => utils.access(options.isEnabled), isEnabled => {
    if (!isEnabled) {
      return;
    }
    if (utils.isIOS()) {
      ...
      solidJs.onCleanup(preventScrollStandard());
    }
  }));
}
...
function preventScrollMobileSafari() {
  ...
  const restoreStyles = utils.chain([setStyle(document.documentElement, "paddingRight", `${window.innerWidth - document.documentElement.clientWidth}px`), setStyle(document.documentElement, "overflow", "hidden"), setStyle(document.body, "marginTop", `-${scrollY}px`)]);
...
function createReducedMotion(fallbackState, watchChange) {
  return media.createMediaQuery("(prefers-reduced-motion: reduce)", fallbackState, watchChange);
}
...
function createTagName(ref, type) {
  const [tagName, setTagName] = solidJs.createSignal(utils.stringOrUndefined(type?.()));
  solidJs.createEffect(() => {
    setTagName(ref()?.tagName.toLowerCase() || utils.stringOrUndefined(type?.()));
  });
  return tagName;
}
...
function getTransitionStyles(params) {
  ...
  if (utils.isString(params.transition)) {
...
exports.DEFAULT_TRANSITIONS_NAMES = DEFAULT_TRANSITIONS_NAMES;
...
exports.createReducedMotion = createReducedMotion;
