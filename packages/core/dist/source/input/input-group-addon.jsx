...
const InputGroupAddon = createHopeComponent(props => {
    ...
    return (<hope.div data-required={dataAttr(context.isRequired())} data-disabled={dataAttr(context.isDisabled())} data-readonly={dataAttr(context.isReadOnly())} data-invalid={dataAttr(context.isInvalid())} class={clsx(context.baseClasses().addon, placementStyleConfig().className, local.class)} __css={{
            ...context.styleOverrides().addon,
            ...placementStyleConfig().styleOverride,
            ...local.__css,
        }} {...others}/>);
});
export const InputGroupLeftAddon = createHopeComponent(props => {
    return <InputGroupAddon addonPlacement="left" {...props}/>;
});
export const InputGroupRightAddon = createHopeComponent(props => {
